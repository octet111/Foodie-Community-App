...
## フェーズ13: 追補 — **完成（2026-06-28）**

店公開設定・精算/実績/未払いのバグ修正。詳細は `implementation_spec.md` §15。

### P13-1. 店リスト — 公開/非公開・みんなの行きたい ✅
`stocks.is_private`。S07「行きたい」内「自分／みんな」。企画済みバッジ。DB `20250626000001`〜`02`。

### P13-2. 精算 — 確定後支払・明細再同期 ✅
`20250626000003` で確定後も支払チェック可。`settlementItemsNeedSync` で参加者変更時に明細同期。

### P13-3. 実績リスト — held 基準 + DB トリガ ✅
`/records` は `events.status=held`。`20250628000001` で確定/取消と status 同期。

### P13-4. マイページ未払い — RPC ✅
`get_my_unpaid_items`（`20260628110036`）。一般ユーザーの未払一覧を RLS 回避で取得。

---

## フェーズ14: 店リスト編集・削除 — **完成（2026-06-28）**

店のエリア・予約難易度編集、行きたいリストからの削除、管理者による全投稿削除。詳細は `implementation_spec.md` §16。

### P14-1. 投稿店の編集・削除 ✅
投稿者（`created_by`）は S07「自分」一覧・S08 で `area`/`rarity` を編集。自分の `stocks` 削除。有効企画なしなら `shops` も削除。

**反映**: `ShopEditModal`, `shop-actions.ts`, `ShopStockCard`, `ShopDetailClient`. DB `20250628120000_shops_delete_creator.sql`.

### P14-2. 管理者による全投稿削除 ✅
admin は「全員」タブで非公開投稿を含む全ストックを表示。「削除（管理者）」で任意の投稿を削除。

**反映**: `getAllStocksExcept()`, `ShopsPageClient`（admin 時タブ名「全員」）, `shop-actions.ts`（`asAdmin` オプション）.

### P14-3. E2E ✅
`shops.spec.ts` — エリア・予約難易度編集、削除。

---

## 実行順サマリ
P0（環境）→ P1（DB/RLS）→ P2（認証）→ P3（シェル）→ P4（店）→ P5（企画）→ P6（精算）→ P7（運用）→ P8（仕上げ）→ P9（UIブラッシュアップ）→ P10（機能ブラッシュアップ）→ P11（機能ブラッシュアップ #3）→ **P12（機能ブラッシュアップ #4）** → **P13（追補）** → **P14（店編集・削除）**

各フェーズ末のE2Eを必ず通してから次へ。⚑ の付いたタスクだけは人間が値・方針を確定させること。

**本番前の残タスク（2026-06-28 時点）**: P8-6 Vercel デプロイ、P8-7 全 AC E2E 網羅、Resend ドメイン認証、`APP_URL` 本番化、migration リモート適用確認（**`20250628120000`**, **`20260628110036`** 含む）。
