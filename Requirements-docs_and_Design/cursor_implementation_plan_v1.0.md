...
**反映**: `ShopsPageClient`。**2026-06-21**: 上部に primary「＋ 店を追加」ボタン（S04 企画作成と同配置）。ストックの `memo` は一覧にテキスト表示のみ。
...
ホーム分離・実績リスト・店メモ・企画 UI・参加取消修正。詳細は `implementation_spec.md` §12 参照。

### P10-1. ホーム・ルーティング分離 ✅
ログイン後 `/` をハブ（企画一覧・店リスト導線）。企画一覧を `/events` に移動。ナビ「企画」→ `/events`。
【完了条件】下部/サイドナビから企画一覧に遷移できる。
**反映**: `page.tsx`, `events/page.tsx`, `HomePageClient.tsx`, `constants.ts`。
...
**反映**: `ShopAddModal`, `ShopDetailClient`, `getUserStockForShop`.
...
## 実行順サマリ
P0（環境）→ P1（DB/RLS）→ P2（認証）→ P3（シェル）→ P4（店）→ P5（企画）→ P6（精算）→ P7（運用）→ P8（仕上げ）→ P9（UIブラッシュアップ）→ P10（機能ブラッシュアップ）→ **P11（機能ブラッシュアップ #3）**

各フェーズ末のE2Eを必ず通してから次へ。⚑ の付いたタスクだけは人間が値・方針を確定させること。

**本番前の残タスク（2026-06-21 時点）**: P8-6 Vercel デプロイ、P8-7 全 AC E2E 網羅、Resend ドメイン認証、`APP_URL` 本番化、migration リモート適用確認（`20250622000001` / `20250622000002` / `20250623000002` / **`20250624000001`**）。
