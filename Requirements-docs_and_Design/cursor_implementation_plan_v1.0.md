...
## 実行順サマリ
P0（環境）→ P1（DB/RLS）→ P2（認証）→ P3（シェル）→ P4（店）→ P5（企画）→ P6（精算）→ P7（運用）→ P8（仕上げ）→ P9（UIブラッシュアップ）→ **P10（機能ブラッシュアップ）**

各フェーズ末のE2Eを必ず通してから次へ。⚑ の付いたタスクだけは人間が値・方針を確定させること。

**本番前の残タスク（2026-06-21 時点）**: P8-6 Vercel デプロイ、P8-7 全 AC E2E 網羅、Resend ドメイン認証、`APP_URL` 本番化、migration リモート適用確認（`20250622000001` / `20250622000002` / **`20250623000002`**）。
